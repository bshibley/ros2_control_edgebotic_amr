...
ArduinoComms::ArduinoComms(const std::string &serial_device, int32_t baud_rate, int32_t timeout_ms, bool debug)
{
    setup(serial_device, baud_rate, timeout_ms, debug);
}
...
void ArduinoComms::setup(const std::string &serial_device, int32_t baud_rate, int32_t timeout_ms, bool debug)
{
    ...
    serial_conn_.setTimeout(tt); // setTimeout takes a reference and so needs a variable
    serial_conn_.open();
}
